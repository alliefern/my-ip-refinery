...
| 27 | Client caps concurrent uploads at 2 and warns on `beforeunload` while uploads are active | The same incident queued all 10 files simultaneously, saturating the connection; nothing warned the user before they likely closed the tab. |
