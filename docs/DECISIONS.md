...
| 29 | Project-level jobs (`build_ip_map`, `generate_blueprint`, `generate_lessons`, `generate_course_assets`) get a manual Retry surfaced on Overview, and contextually on IP Map / Course Direction | These jobs have no source asset and, unlike upload/transcription failures, previously had zero UI recourse once they hit MAX_ATTEMPTS — a thin library tripping the IP map's validation left the project permanently stuck with nothing clickable. |
| 30 | build_ip_map failures show a plain-language "not enough footage" message only for `INVALID_MODEL_OUTPUT`/`INSUFFICIENT_SOURCE`; other error codes (rate limit, timeout) show the real error | Owner-requested copy is accurate for the common case (a thin library) but would misdiagnose an API hiccup as a content problem if shown unconditionally. |
