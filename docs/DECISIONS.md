...
| 27 | Uploads stay uncapped (all files fire in parallel) after upgrading Supabase to the paid tier; the `beforeunload` warning stays | Owner's explicit call — the client's own upload bandwidth is the real constraint on concurrency, not Supabase capacity, and she wants the whole batch to start immediately regardless. |
